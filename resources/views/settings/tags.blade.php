              ...
              @foreach (auth()->user()->company->tags as $tag)
                <li class="table-row" data-tag-id="{{ $tag->id }}">
                  <div class="table-cell">
                    {{ $tag->name }}
                    <span
                      class="tags-list-contact-number">({{ trans('settings.tags_list_contact_number', ['count' => $tag->contacts()->count()]) }}
                      )</span>
                  </div>
                  <div class="table-cell actions">
                    <a href="/settings/tags/{{ $tag->id }}/delete"
                       onclick="return confirm('{{ trans('settings.tags_list_delete_confirmation') }}')">
