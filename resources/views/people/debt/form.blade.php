<form method="POST" action="{{ $action }}">
  ...
  <fieldset class="form-group">
    <label class="form-check-inline" for="youowe">
      <input type="radio" class="form-check-input" name="in_debt" id="youowe" value="yes"
             @if(old('in_debt') !== 'no' || $debt->in_debt !== 'no') checked @endif>
  ...
  <div class="form-group">
    ...
    <input type="number" class="form-control" name="amount" id="amount" maxlength="254"
           value="{{ old('amount') ?? $debt->amount }}" autofocus required>
  </div>
  ...
  <div class="form-group">
    ...
    <textarea class="form-control" name="reason" id="reason"
              maxlength="2500">{{ old('reason') ?? $debt->reason }}</textarea>
  </div>
